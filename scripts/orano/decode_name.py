...
# ######## Intialize log #################
# ##################################
logging.basicConfig(filename='log.log', format="%(asctime)s || %(funcName)s || %(levelname)s || %(message)s",
                    level=logging.DEBUG)
...
# ##################################
    ...
    if code_country == "GA": #exception for Gabon GA_C__KAYA_XX_XXXX_XXXXX_____01 → Country = Gabon Region = C_KAYA_KAYA
...
# ##################################
    ...
    if name_country :
        logging.info(
            "Country = {}".format(name_country))
    else:
        logging.error("No country found")

    if name_region :
        logging.info(
        "Country {} an Region {} have been extracted.".format(name_country,name_region))
    else:
...
# ##################################
        ...
        for maintheme in mainthemes:
            if code_maintheme == maintheme.get('code'):
                ...
                for subtheme in maintheme:
                    ...
                    elif name[25:28] == subtheme.get('code'): #Subtheme exceptions (when theme 2 composed by 3 characters)
                        logging.warning("Theme 2 containt 3 characters, so  there is no theme 3")
                        name_theme2 = subtheme.get('desc')
                        code_theme2 = name[25:28]
                    elif name[25:29] == subtheme.get('code'): #Subtheme exceptions (when theme 2 composed by 4 characters)
                        logging.warning("Theme 2 containt 4 characters, so there is no theme 3")
                        name_theme2 = subtheme.get('desc')
            ...
            if code_maintheme == maintheme.get('code'):
                ...
                for subtheme in maintheme:
                    if code_theme2 == subtheme.get('code'): 
                        ...
                        name_theme3 = subtheme.get('desc')
                    elif name[22:25] == subtheme.get('code'): #Subtheme exceptions (when theme 2 composed by 3 characters)
                        logging.warning("Theme 2 containt 3 characters, so there is no theme 3")
                        name_theme2 = subtheme.get('desc')
                        code_theme2 = name[22:25]
                    elif name[22:26] == subtheme.get('code'): #Subtheme exceptions (when theme 2 composed by 4 characters)
                        logging.warning("Theme 2 containt 4 characters, so there is no theme 3")
                        name_theme2 = subtheme.get('desc')
    ...
    if name_maintheme:
        logging.info(
            "Main theme = {}".format(name_maintheme))
    else:
        logging.error("No Main theme found")

    if name_region :
        logging.info(
            "Main theme is {} and Subtheme(s) are {} and {} ".format(name_maintheme,name_theme2,name_theme3))
    else:
...
if __name__ == "__main__":
    ...
    with open('result.csv', 'w', newline='') as csvfile:
        fieldnames = ["Name","Country","Region","Main Theme","Year","Scale","Theme 2","Theme 3", "Path"]
        
        csv.register_dialect('semicolon', delimiter=';') #create dialect

        writer = csv.DictWriter(csvfile, fieldnames=fieldnames, dialect='semicolon') #write csv with a dictionnary
        ...
        for dict_name_path in list_names_path:
            ...
            if country is None : #if the country can't be found, the name might be undecodable
                logging.error("Name {} might be undecodable and will no be added to CSV Result".format(result.get("Name")))
    ...
    logging.info("{} filenames have been added to CSV Result, {} not.".format(counter_csv, counter_undecodable))